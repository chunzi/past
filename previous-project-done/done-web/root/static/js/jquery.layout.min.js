...
fn=eval(v_fn);if(typeof fn=="function")fn(pane,$Ps[pane],$.extend({},state[pane]),$.extend({},options[pane]),options.name);}catch(ex){}};var cssNum=function($E,prop){var
...
CSS={overflow:"hidden"},p=$Container.css("position"),h=$Container.css("height");if(!p||"fixed,absolute,relative".indexOf(p)<0)CSS.position="relative";if(!h||h=="auto")CSS.height="100%";$Container.css(CSS);}}catch(ex){}cDims=state.container=getElemDims($Container);};var initHotkeys=function(){$.each(c.borderPanes.split(","),function(i,pane){var o=options[pane];if(o.enableCursorHotkey||o.customHotkey){$(document).keydown(keyDown);return false;}});};var initOptions=function(){opts=transformData(opts);$.each("name,scrollToBookmarkOnLoad".split(","),function(idx,key){if(opts[key]!==undefined)options[key]=opts[key];else if(opts.defaults[key]!==undefined){options[key]=opts.defaults[key];delete opts.defaults[key];}});$.each("paneSelector,resizerCursor,customHotkey,onopen,onclose,onresize".split(","),function(idx,key){delete opts.defaults[key];});var fx=c.fxDefaults;if(opts.fxDefaults)$.extend(fx,opts.fxDefaults);c.center=$.extend(true,{},c.defaults,c.center);$.extend(options.center,opts.center);var o_Center=$.extend(true,{},options.defaults,opts.defaults,options.center);$.each("paneClass,contentSelector,contentIgnoreSelector,applyDefaultStyles,showOverflowOnHover".split(","),function(idx,key){options.center[key]=o_Center[key];});$.each(c.borderPanes.split(","),function(i,pane){c[pane]=$.extend(true,{},c.defaults,c[pane]);var o=options[pane]=$.extend(true,{},options.defaults,options[pane],opts.defaults,opts[pane]);if(!o.paneClass)o.paneClass=defaults.paneClass;if(!o.resizerClass)o.resizerClass=defaults.resizerClass;if(!o.togglerClass)o.togglerClass=defaults.togglerClass;$.each("onopen,onclose,onresize".split(","),function(idx,key){if(o[key]==undefined)o[key]="";});var fxName=o.fxName;if(fxName!="none"){if(!fxName||!$.effects||!$.effects[fxName]||(!fx[fxName]&&!o.fxSettings))o.fxName="none";else if(fx[fxName])o.fxSettings=$.extend({},fx[fxName].all,fx[fxName][pane],o.fxSettings);}});$.extend(options.defaults,opts.defaults);};var initPanes=function(){$.each(c.allPanes.split(","),function(){var
...
edge=c[pane].edge,isOpen=$P.is(":visible"),spacing=(isOpen?o.spacing_open:o.spacing_closed),_side="-"+pane,_state=(isOpen?"-open":"-closed"),$R,$T;$R=$Rs[pane]=$("<span></span>");if(isOpen&&o.resizable);else if(!isOpen&&o.slidable)$R.attr("title",o.sliderTip).css("cursor",o.sliderCursor);$R.attr("id",(o.paneSelector.substr(0,1)=="#"?o.paneSelector.substr(1)+"-resizer":"")).attr("resizer",pane).css(c.resizers.cssReq).css(edge,cDims[edge]+getPaneSize(pane)).addClass(rClass+" "+rClass+_side+" "+rClass+_state+" "+rClass+_side+_state).appendTo($Container);if(o.applyDefaultStyles)$R.css(c.resizers.cssDef);if(o.closable){$T=$Ts[pane]=$("<div></div>");$T.attr("id",(o.paneSelector.substr(0,1)=="#"?o.paneSelector.substr(1)+"-toggler":"")).css(c.togglers.cssReq).attr("title",(isOpen?o.togglerTip_open:o.togglerTip_closed)).click(function(evt){toggle(pane);evt.stopPropagation();}).mouseover(function(evt){evt.stopPropagation();}).addClass(tClass+" "+tClass+_side+" "+tClass+_state+" "+tClass+_side+_state).appendTo($R);if(o.togglerContent_open)$("<span>"+o.togglerContent_open+"</span>").addClass("content content-open").css("display",s.isClosed?"none":"block").appendTo($T);if(o.togglerContent_closed)$("<span>"+o.togglerContent_closed+"</span>").addClass("content content-closed").css("display",s.isClosed?"block":"none").appendTo($T);if(o.applyDefaultStyles)$T.css(c.togglers.cssDef);if(!isOpen)bindStartSlidingEvent(pane,true);}});sizeHandles("all",true);};var initResizable=function(){var
...
s=state[pane],$P=$Ps[pane],$R=$Rs[pane];if(!$P)return;else if(s.isHidden)return;else s.isHidden=true;s.isSliding=false;if($R)$R.hide();if(onInit){$P.hide();s.isClosed=true;}else close(pane,true);};var show=function(pane,openPane){var
o=options[pane],s=state[pane],$P=$Ps[pane],$R=$Rs[pane];if(!$P)return;else if(!s.isHidden)return;else s.isHidden=false;s.isSliding=false;if($R&&o.spacing_open>0)$R.show();if(openPane===false)close(pane,true);else
...
$P=$Ps[pane],$R=$Rs[pane],$T=$Ts[pane],o=options[pane],s=state[pane],doFX=!noAnimation&&!s.isClosed&&(o.fxName!="none"),edge=c[pane].edge,rClass=o.resizerClass,tClass=o.togglerClass,_side="-"+pane,_open="-open",_sliding="-sliding",_closed="-closed",cP;if(!$P||(!o.resizable&&!o.closable))return;else if(!force&&s.isClosed)return;if(c.isLayoutBusy){setFlowCallback("close",pane,force);return;}else{cP=c[pane];cP.isMoving=true;c.isLayoutBusy=true;}s.isClosed=true;syncPinBtns(pane,false);if(!s.isSliding)sizeMidPanes(c[pane].dir=="horz"?"all":"center");if($R){$R.css(edge,cDims[edge]).removeClass(rClass+_open+" "+rClass+_side+_open).removeClass(rClass+_sliding+" "+rClass+_side+_sliding).addClass(rClass+_closed+" "+rClass+_side+_closed);if(o.resizable)$R.draggable("disable").css("cursor","default").attr("title","");if($T){$T.removeClass(tClass+_open+" "+tClass+_side+_open).addClass(tClass+_closed+" "+tClass+_side+_closed).attr("title",o.togglerTip_closed);}sizeHandles();}if(doFX){lockPaneForFX(pane,true);$P.hide(o.fxName,o.fxSettings,o.fxSpeed,function(){lockPaneForFX(pane,false);if(!s.isClosed)return;close_2();});}else{$P.hide();close_2();}function close_2(){bindStartSlidingEvent(pane,true);execUserCallback(pane,o.onclose);execFlowCallback(pane);}};var open=function(pane,slide,noAnimation){var
$P=$Ps[pane],$R=$Rs[pane],$T=$Ts[pane],o=options[pane],s=state[pane],doFX=!noAnimation&&s.isClosed&&(o.fxName!="none"),edge=c[pane].edge,rClass=o.resizerClass,tClass=o.togglerClass,_side="-"+pane,_open="-open",_closed="-closed",_sliding="-sliding",cP;if(!$P||(!o.resizable&&!o.closable))return;else if(!s.isClosed&&!s.isSliding)return;if(c.isLayoutBusy){setFlowCallback("open",pane,slide);return;}else{cP=c[pane];cP.isMoving=true;c.isLayoutBusy=true;}if(s.isSliding&&!slide)bindStopSlidingEvents(pane,false);s.isClosed=false;s.isHidden=false;setPaneMinMaxSizes(pane);if(s.size>s.maxSize)$P.css(c[pane].sizeType,max(1,cssSize(pane,s.maxSize)));bindStartSlidingEvent(pane,false);if(doFX){lockPaneForFX(pane,true);$P.show(o.fxName,o.fxSettings,o.fxSpeed,function(){lockPaneForFX(pane,false);if(s.isClosed)return;open_2();});}else{$P.show();open_2();}function open_2(){if(!s.isSliding)sizeMidPanes(c[pane].dir=="vert"?"center":"all");if($R){$R.css(edge,cDims[edge]+getPaneSize(pane)).removeClass(rClass+_closed+" "+rClass+_side+_closed).addClass(rClass+_open+" "+rClass+_side+_open).addClass(!s.isSliding?"":rClass+_sliding+" "+rClass+_side+_sliding);if(o.resizable)$R.draggable("enable").css("cursor",o.resizerCursor).attr("title",o.resizerTip);else
$R.css("cursor","default");if($T){$T.removeClass(tClass+_closed+" "+tClass+_side+_closed).addClass(tClass+_open+" "+tClass+_side+_open).attr("title",o.togglerTip_open);}sizeHandles("all");}sizeContent(pane);syncPinBtns(pane,!s.isSliding);execUserCallback(pane,o.onopen);execFlowCallback(pane);}};var lockPaneForFX=function(pane,doLock){var $P=$Ps[pane];if(doLock){$P.css({zIndex:c.zIndex.animation});if(pane=="south")$P.css({top:cDims.top+cDims.innerHeight-$P.outerHeight()});else if(pane=="east")$P.css({left:cDims.left+cDims.innerWidth-$P.outerWidth()});}else{if(!state[pane].isSliding)$P.css({zIndex:c.zIndex.pane_normal});if(pane=="south")$P.css({top:"auto"});else if(pane=="east")$P.css({left:"auto"});}};var bindStartSlidingEvent=function(pane,enable){var
